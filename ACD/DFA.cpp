...
void query(struct trie* root,string imagePath,int blockNumber,int perc){
	...
	for(int i=0;i<(int)(root->nodeList.size());i++)
	{
		...
		{
		 return	 query(root->nodeList[i].second,imagePath,blockNumber+1,perc);
		}
	}
	return ;
}
...
{  
	...
	scanf("%d",&img);
	 string str="./images2/"+to_string(img)+".jpg";
	 cout<<"Quering the string "<<str <<" : ";
	 query(rootaddr,str,1,100);
	 cout<<endl;
	...
	time_end = TimeValue_Final.tv_sec * 1000000 + TimeValue_Final.tv_usec;
	time_overhead = (time_end - time_start)/1000000.0;
	printf("Parallel Computation time to query is %lf\n",time_overhead);	
	}
	return 0;
}